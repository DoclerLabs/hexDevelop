...
namespace EditorConfig.Core
{
    ...
    public class EditorConfigMinimatcher
    {
        ...
        public static Func<string, bool> CreateFilter(string pattern, EditorConfigMinimatcherOptions options = null)
        {
            ...
            // "" only matches ""
            if (StringHelper.IsNullOrWhiteSpace(pattern)) return String.IsNullOrEmpty;
        ...
        public static bool Check(string input, string pattern, EditorConfigMinimatcherOptions options = null)
        {
            ...
            // "" only matches ""
            if (StringHelper.IsNullOrWhiteSpace(pattern)) return input == "";
        ...
        Regex MakeRegex()
        {
            ...
            var re = String.Join("|", set.Select(pattern =>
                  String.Join("\\/", pattern.Select(p => p.RegexSource(options)).ToArray()
              )).ToArray());
